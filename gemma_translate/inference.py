...
class MoonshineStatic(MoonshineStaticBase):
    ...
    def close(self):
        """Explicitly release all IREE inference runners to prevent nanobind leak warnings on exit."""
        self._kv_cache = None
        for runner in (self._preprocessor, self._encoder, self._decoder, self._decoder_with_past):
